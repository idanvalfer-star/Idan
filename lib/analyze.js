...
function prompt(platform, evidence) {
  return `You are analysing a cooking/recipe video from ${platform}. From the evidence below (spoken transcript, caption, and any still frames), return three things:

1. name: CRITICAL — ONLY 2-3 words MAXIMUM. JUST the core dish name, nothing else (e.g. "Thai Noodles", "Fluffy Pancakes", "Cheeseburger"). NEVER use adjectives (delicious, classic, crispy, creamy, style, featured, fried). NEVER describe the dish. ONLY the dish type and ONE basic ingredient if essential. Examples: "Garlic Chicken", not "A classic garlic chicken". "Thai Noodles", not "Thai stir-fried noodle dish".
2. description: ONE short, concise sentence describing the dish. Keep it brief and easy to scan.
3. ingredients: every grocery ingredient the recipe needs, with a quantity when one is stated ("2 cups", "1 tbsp", "a pinch"). Use "" for quantity when none is given. Use plain ingredient names ("olive oil", not "extra virgin olive oil drizzle"). No equipment, steps, or non-grocery items. Deduplicate.

If the material clearly is not a recipe, return an empty ingredients list, a best-effort name, and say so in the description.

Respond ONLY with JSON of the form:
{"name": string, "description": string, "ingredients": [{"name": string, "quantity": string}]}
...
// A single call to one model.
async function geminiCall({ geminiKey, model, parts }) {
  ...
  if (!resp.ok) {
    ...
    // Quota, rate, or model-availability problems: worth trying a different model.
    const retry = resp.status === 429 || resp.status === 404
      || /quota|exceeded|limit:\s*0|rate|not found|not available|no longer available|update your code|unsupported|deprecat/i.test(msg);
    e.code = retry ? "GEMINI_RETRY" : "GEMINI_FAILED"; // GEMINI_FAILED (e.g. bad key) stops immediately
    throw e;
  }
  const j = await resp.json();
...
// Ask Google which models THIS key can actually use, and pick a video-capable
// flash model. Cached after the first successful lookup. Adapts automatically as
// Google adds/removes models, instead of hardcoding names that come and go.
let _geminiModel = null;
async function resolveGeminiModel(geminiKey, preferred) {
  if (_geminiModel) return _geminiModel;
  try {
    const r = await fetch(`https://generativelanguage.googleapis.com/v1beta/models?key=${encodeURIComponent(geminiKey)}&pageSize=1000`);
    if (r.ok) {
      const j = await r.json();
      const names = (j.models || [])
        .filter(m => (m.supportedGenerationMethods || []).includes("generateContent"))
        .map(m => String(m.name || "").replace(/^models\//, ""));
      const flash = names.filter(n => /flash/.test(n) && !/thinking|vision|exp|preview|8b/i.test(n));
      const chosen =
        (preferred && names.includes(preferred) && preferred) ||
        flash.find(n => /^gemini-2\.0-flash$/.test(n)) ||
        flash.find(n => /^gemini-flash-latest$/.test(n)) ||
        flash.find(n => /^gemini-2\.5-flash$/.test(n)) ||
        flash.find(n => /flash-latest/.test(n)) ||
        flash[0] || names[0];
      if (chosen) { _geminiModel = chosen; return chosen; }
    }
  } catch { /* fall through to preferred */ }
  return preferred || "gemini-2.0-flash";
}
// Try a resolved model, then fall back through others on quota/availability errors.
async function geminiGenerate({ geminiKey, model, parts }) {
  const primary = await resolveGeminiModel(geminiKey, model);
  const order = [...new Set([primary, model, "gemini-2.0-flash", "gemini-flash-latest",
    "gemini-2.5-flash-lite", "gemini-2.0-flash-lite", "gemini-2.5-flash", "gemini-1.5-flash"].filter(Boolean))];
  let lastErr;
  for (const m of order) {
    try { return await geminiCall({ geminiKey, model: m, parts }); }
    catch (e) {
      lastErr = e;
      if (e.code !== "GEMINI_RETRY") throw e; // bad key / unparseable → stop
      _geminiModel = null; // this model didn't work; don't stick to it
    }
  }
  throw lastErr;
}
...
export async function analyzeVideo(url, { anthropicKey, openaiKey, geminiKey, geminiModel } = {}) {
  ...
  try {
    ...
    const { name, description, ingredients } = provider === "gemini"
