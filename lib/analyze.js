...
async function geminiCall({ geminiKey, model, parts }) {
  ...
  if (!resp.ok) {
    ...
    // Quota / rate errors are worth retrying on a different model.
    e.code = (resp.status === 429 || /quota|exceeded|limit:\s*0|rate/i.test(msg)) ? "GEMINI_QUOTA" : "GEMINI_FAILED";
    throw e;
  }
  const j = await resp.json();
...
// Try the chosen model, then fall back to other free models if one is quota-capped.
async function geminiGenerate({ geminiKey, model, parts }) {
  const order = [...new Set([model, "gemini-2.5-flash", "gemini-2.0-flash", "gemini-2.0-flash-lite", "gemini-2.5-flash-lite", "gemini-1.5-flash"])];
  let lastErr;
  for (const m of order) {
    try { return await geminiCall({ geminiKey, model: m, parts }); }
    catch (e) {
      lastErr = e;
      if (e.code !== "GEMINI_QUOTA") throw e; // only quota errors are worth trying another model
    }
  }
  throw lastErr;
}
